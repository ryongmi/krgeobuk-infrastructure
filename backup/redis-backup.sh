...
echo "Time: ${DATE}"
echo "Backup Directory: ${REDIS_BACKUP_DIR}"
echo "========================================="
...
# dump.rdb 파일 복사
docker cp krgeobuk-redis:/data/dump.rdb "${REDIS_BACKUP_DIR}/dump_${DATE}.rdb"

if [ $? -eq 0 ]; then
  echo "✓ Redis backup completed: dump_${DATE}.rdb"

  # 압축
  gzip "${REDIS_BACKUP_DIR}/dump_${DATE}.rdb"
...
ls -lh "${REDIS_BACKUP_DIR}"/*.rdb.gz 2>/dev/null | tail -5 || echo "No backups found"
